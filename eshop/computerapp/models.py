...
class Manufacturer(models.Model):
    ...
    logo = models.ImageField(blank=True, null=True, max_length=200,
                             upload_to='manufacturer/uploads/%Y/%m/%d') #名字最大长度200
    created = models.DateTimeField(auto_now_add=True)
    updated = models.DateTimeField(auto_now=True)
...
class Product(models.Model):
    ...
    description = models.TextField()
    image = models.ImageField(max_length=200, upload_to='product/uploads/%Y/%m/%d') #名字最大长度200
    price = models.DecimalField(max_digits=12, decimal_places=2)
    sold = models.PositiveIntegerField(default=0) # sold number
    category = models.ForeignKey(Category, related_name='product_in', on_delete=models.CASCADE)
    # CASCADE, 级联删除。表示删除一个，与之有关的内容全部删除
...
class DeliveryAddress(models.Model):
    ...
    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE,
                             related_name='delivery_address_of', ) # settings.Auth...是Django自带的用户模型
